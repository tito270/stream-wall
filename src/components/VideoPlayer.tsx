...
const HLS_BASE = (import.meta.env.VITE_HLS_BASE?.replace(/\/+$/, '')) || API_BASE.replace(/\/+$/, '');
...
const VideoPlayerMemo: React.FC<VideoPlayerProps> = ({
  ...
  const [levels, setLevels] = useState<Array<{height?: number; bitrate?: number; name?: string}>>([]);
  ...
  const teardownPlayer = useCallback((video?: HTMLVideoElement | null) => {
    try {
      if (hlsRef.current) {
    ...
    if (Hls.isSupported()) {
      ...
      hls.on(Hls.Events.ERROR, (_event, data) => {
        if (data.fatal) {
          ...
          // Consider more specific retry logic here
        }
      });
  ...
  }, [streamUrl, getStreamType, handleError, proxiedHlsUrl, status, streamId, onBitrateUpdate, isVisible, teardownPlayer]);
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "absolute top-2 left-2 z-10 px-2 py-1 rounded text-xs font-mono flex items-center gap-2",
